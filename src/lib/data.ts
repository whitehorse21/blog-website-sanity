...
export async function getFeaturedPosts(): Promise<Post[]> {
  ...
  return posts.length > 0 ? posts : mockPosts.filter((p) => p.featured).slice(0, 3);
}
...
export async function getRelatedPosts(slug: string, categoryIds: string[]): Promise<Post[]> {
  if (!isSanityConfigured()) {
  ...
  return client.fetch(relatedPostsQuery, { slug, categoryIds });
}
